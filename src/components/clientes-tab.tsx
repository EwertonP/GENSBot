          ...
          {visiveis.map((c) => {
            ...
            return (
              <Card
                ...
                className={`flex flex-col gap-5 ${c.ativo ? '' : 'opacity-60'}`}
              >
                <div className="flex items-center gap-3.5">
                  <ClienteAvatar nome={c.nome} cor={c.cor} fotoUrl={conta?.profile_picture_url} tamanho="md" />
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate text-base font-semibold tracking-tight text-foreground">{c.nome}</h3>
                    {c.nicho && <p className="truncate text-xs text-muted-foreground mt-0.5">{c.nicho}</p>}
                  </div>
                  {!c.ativo && <Badge variant="muted">Arquivado</Badge>}
                </div>

                <div className="flex items-center justify-between gap-3 border-t border-dashed border-border pt-3.5">
                  <span className="flex min-w-0 items-center gap-1.5 text-xs font-semibold text-muted-foreground">
                    <InstagramGlyph className="w-3.5 h-3.5 shrink-0" />
